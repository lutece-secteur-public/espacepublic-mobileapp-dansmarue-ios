...
//  DansMaRue
//
//  Created by alaeddine.oueslati on 21/09/2023.
...
class FloatingLabelInput: UITextField {
    ...
    @IBInspectable
    var floatingLabelFont: UIFont = .systemFont(ofSize: 14) {
        didSet {
    ...
    @objc func addFloatingLabel() {
        if self.text == "" {
            ...
            self.floatingLabel.layer.backgroundColor = .none
            ...
            self.floatingLabel.bottomAnchor.constraint(equalTo: self.topAnchor, constant: -4).isActive = true // Place our label 10 pts above the text field
            self.placeholder = ""
        }
